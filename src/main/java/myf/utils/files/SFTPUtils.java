...
public class SFTPUtils {
                                          ...
                                          String fileFormat, String fileEndFormat, boolean del)
    {
        ...
        try
        {
             //进行登录
             login();
            Vector v = listFiles(remotePath);    //得到目下的所有的文件的列表，返回的是所有文件的对象
            // sftp.cd(remotePath);
            if (v.size() > 0)                    //目录下的个数
            {
                ...
                while (it.hasNext())
                {
                    ...
                    String filename = entry.getFilename();          //获取文件的名字
                    SftpATTRS attrs = entry.getAttrs();
                    if (!attrs.isDir())
                    {
                        boolean flag = false;
                        String localFileName = localPath + filename;      //获取服务路径加上文件名
                        ...
                        {
                            //filename.startsWith()；方法用于判断指定的前缀
                                   ...
                                   boolean del)
    {
        try
        {
            ...
            {
                if (files[i].isFile()
                        && files[i].getName().indexOf("bak") == -1)  //判断文件的格式
                {
